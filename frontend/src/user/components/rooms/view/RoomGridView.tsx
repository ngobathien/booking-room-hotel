...
import { STATUS_ROOM_CONFIG, type Room } from "../../../../types/room.types";
...
export const RoomGridView: React.FC<RoomGridViewProps> = ({ rooms }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {rooms.map((room) => {
        const config = STATUS_ROOM_CONFIG[room.status];

        return (
          <div
            key={room._id}
            className={`group overflow-hidden rounded-3xl bg-white shadow-sm border border-slate-100 flex flex-col transition ${
              config.faded ? "opacity-50" : "hover:shadow-lg"
            }`}
          >
            {/* Thumbnail */}
            <div className="relative h-64 w-full overflow-hidden">
              ...
              {/* Status */}
              <div
                className={`absolute left-4 top-4 rounded-full px-3 py-1 text-xs font-bold text-white ${config.class}`}
              >
                {config.label}
              </div>
            </div>
            ...
            <div className="flex-1 p-6 flex flex-col justify-between">
              ...
              <div className="mt-6">
                ...
                <div className="mt-6 flex gap-3">
                  ...
                  {/* 🔥 Disable nếu không khả dụng */}
                  {config.faded ? (
                    <button
                      disabled
                      className="flex-1 rounded-xl bg-gray-300 py-3 text-center text-sm font-bold text-white cursor-not-allowed"
                    >
                      Không khả dụng
                    </button>
                  ) : (
