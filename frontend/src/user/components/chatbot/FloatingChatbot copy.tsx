...
const FloatingChatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "model",
      text: "Tôi hỗ trợ bạn về phòng, đơn đặt và thanh toán.",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSend = async (messageArg?: string) => {
    const userMessage = (messageArg ?? input).trim();
    ...
    try {
      // Gọi đến API NestJS của bạn (AuthGuard sẽ lấy user từ JWT)
      ...
      const aiResponse: Message = {
        role: "model",
        text: response.data, // Backend trả string
        timestamp: new Date(),
      };
  ...
  const handleSuggestion = (text: string) => {
    // send directly using messageArg to avoid race with setInput
    handleSend(text);
  };
            ...
            <div className="p-4 bg-white border-t border-slate-100">
              <div className="space-y-2">
                <div className="flex gap-2">
                  <button
                    onClick={() => handleSuggestion("Phòng nào còn trống?")}
                  ...
                  <button
                    onClick={() =>
                      handleSuggestion("Tôi có đơn đặt nào không?")
                    }
                    className="px-3 py-1 rounded-full bg-slate-100 text-sm hover:bg-slate-200"
                  >
                    Tôi có đơn đặt nào không?
                  </button>
                  <button
                    onClick={() => handleSuggestion("Tôi đã thanh toán chưa?")}
                    className="px-3 py-1 rounded-full bg-slate-100 text-sm hover:bg-slate-200"
                  >
                ...
                <div className="relative flex items-center gap-2 bg-slate-100 rounded-2xl p-1.5 focus-within:ring-2 focus-within:ring-blue-600/20 transition-all">
                  <input
                    ...
                    placeholder="Hỏi về phòng, đơn đặt hoặc thanh toán..."
